/*
  ...
  https://github.com/michaelshiloh/resourcesForClasses/blob/master/kicad/nRF_servo_Mega
   ...
   Search for the phrase CHANGE HERE to see where to 
...
// Servo motors
const int WAIST_SERVO_PIN = 19;
const int ELBOWLEFT_SERVO_PIN = 20;
const int ELBOWRIGHT_SERVO_PIN = 21;
...
Servo waist;  // change names to describe what's moving
Servo elbowleft;
Servo elbowright;
// Servo tail;
// Servo grabber;
...
const int WAIST_LEFT = 50;
const int WAIST_RIGHT = 90;
const int WAIST_FORWARD = 65;
//
const int ELBOW_LEFT = 190;
const int ELBOW_LEFT_STRAIGHT = 120;
const int ELBOW_RIGHT = 0;
const int ELBOW_STRAIGHT = 60;
...
void setupServoMotors() {
  ...
  waist.write(WAIST_FORWARD);

  elbowleft.attach(ELBOWLEFT_SERVO_PIN);
  elbowleft.write(ELBOW_LEFT_STRAIGHT);

  elbowright.attach(ELBOWRIGHT_SERVO_PIN);
  elbowright.write(ELBOW_STRAIGHT);
...
void loop() {
  ...
  if (radio.available(&pipeNum)) {
    ...
    switch (data.stateNumber) {
      ...
      case 1: //waist rotation
        for (int i = 0; i < 2; i++) {
          waist.write(WAIST_RIGHT);
          delay(900);
          waist.write(WAIST_LEFT);
          delay(900);
        }
      ...
      case 2: //hand clacking motion 
        
        elbowright.write(ELBOW_RIGHT);
        elbowleft.write(ELBOW_LEFT);
        delay(900);
...
case 3: //dance move
        ...
        // move both arms back
        elbowright.write(ELBOW_LEFT);
        elbowleft.write(ELBOW_STRAIGHT);
        delay(900);
